...
Giving priority:
If you place no ordinary signals or stations on one branch, or you place them too far from the intersection (see the notes), trains entering the intersection from that branch will not give way. If you place an ordinary signal too far from an intersection, trains passing it will grind to a halt. This is ugly; to fix it, ask UG to give us an api to turn signals red.

[b]Notes:[/b]
1) Priority signals are expensive, only use them where you really need them.
2) Switch priority computations on and off with the icon in the bottom bar. When they are off, priority signals will behave like standard signals. When you turn them on, it might take a few seconds before priorities start working.
3) Tracks are made of subsequent segments, you can check them with debug mode and <AltGr + L>. If a signal is too many segments apart from an intersection, never mind how short they are, the priority computation will ignore it. This is inconvenient but gives better performance. Priority signals must be within ]] .. constants.maxNSegmentsBeforeIntersection .. [[ segments between one another, or from the intersection. Ordinary signals or stations must be within ]] .. constants.maxNSegmentsBehindIntersection .. [[ segments away from the intersection.
...
5) Every branch linked to an intersection with priority signals needs a signal (of any type) or a station near the intersection.
6) When you start a game, it might take a few seconds before priorities start working.
