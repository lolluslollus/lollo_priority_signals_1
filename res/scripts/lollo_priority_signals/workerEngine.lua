    ...
    mp.path.edges[mp.dyn.pathPos.edgeIndex + 1] =
    {
        ...
        __doc__ = {
            new = "(EdgeId edgeId, Bool dir) -> EdgeIdDir",
                ...
                local _getVehicleIdsNearPrioritySignals = function(nodeEdgeBeforeIntersection_indexedBy_inEdgeId)
                ...
                local _isAnyTrainBoundForEdgeOrNodeId1 = function(vehicleIds_indexed, edgeId)
                ...
                local _isAnyTrainBoundForEdgeOrNodeId2 = function(vehicleIds_indexed, edgeOrNodeId)
                    -- in the following, false means "only occupied now", true means "occupied nor or soon"
                    -- "soon" means "since a vehicle left the last station and before it reaches the next"
                ...
                local _isAnyTrainBoundForEdgeOrNodeIdId = _isAnyTrainBoundForEdgeOrNodeId2
                ...
                for intersectionNodeId, nodeEdgeBeforeIntersection_indexedBy_inEdgeId in pairs(nodeEdgeBeforeIntersection_indexedBy_intersectionNodeId_edgeIdHavingWay) do
                    ...
                    if hasVehicleIdsNearPrioritySignals then
                        for edgeIdGivingWay, nodeEdgeIdBehindIntersection in pairs(nodeEdgeBehindIntersection_indexedBy_intersectionNodeId_edgeIdGivingWay[intersectionNodeId]) do
                            if not(_isAnyTrainBoundForEdgeOrNodeIdId(vehicleIdsNearPrioritySignals, edgeIdGivingWay))
                            -- LOLLO TODO if I have a cross, I should check the nodes, the edges won't do. Check if the following works.
                            and not(_isAnyTrainBoundForEdgeOrNodeIdId(vehicleIdsNearPrioritySignals, nodeEdgeIdBehindIntersection.nodeIdTowardsIntersection))
